...
% runs (8 runtimes × RQ1..RQ4). Numbers flagged "[Preliminary — full run pending]"
...
\emph{Preprint status.} This is a v1 preprint. §6 reports population-ratio $\Delta$ from a partial cross-runtime sweep (8 runtimes across RQ1--RQ5); remaining runtimes and full-suite repeats will be released in v2.
\end{abstract}
...
\textbf{Runs.} We report eight full runs on the EALE benchmark, organized into three base-runtime families so that every cross-runtime observation stays inside a controlled comparison:
\begin{itemize}
...
\textbf{Pass rate is saturated on EALE.} Across all eight runs, Base pass rate is 0.99--1.00 and Loaded pass rate is 0.99--1.00 (Table~\ref{tab:rq1}). Pass rate is therefore a low-resolution signal of self-evolution on this benchmark; the primary lens is interaction efficiency, with pass rate reported for transparency and to flag any regression.
...
\begin{axis}[
    ...
    symbolic x coords={OpenClaw, Hermes, GenericAgent},
    xtick=data,
...
% implicit base
\addplot[fill=gray!55, draw=gray!70!black] coordinates {(OpenClaw,2.74) (Hermes,-17.24) (GenericAgent,-1.75)};
% openspace / (active for genericagent)
\addplot[fill=blue!55, draw=blue!70!black] coordinates {(OpenClaw,-3.95) (Hermes,-19.09) (GenericAgent,1.92)};
% agentmemory (genericagent has none -> plotted as 0-height/omitted via nan)
\addplot[fill=orange!70, draw=orange!80!black] coordinates {(OpenClaw,2.28) (Hermes,-16.64) (GenericAgent,nan)};
\legend{implicit base, \texttt{+openspace} (\texttt{+active} for GA), \texttt{+agentmemory}}
\draw[very thick, black] (axis cs:OpenClaw,0) -- (axis cs:GenericAgent,0);
\node[anchor=north east, font=\footnotesize\bfseries, text=green!45!black, align=right] at (axis cs:GenericAgent,7.3) {$\downarrow$ below zero $=$ fewer turns $=$ \emph{better}};
\end{axis}
\end{tikzpicture}
\caption{\textbf{RQ1 at a glance: $\Delta$Turns\% by base family} (from Table~\ref{tab:rq1}; the third GenericAgent bar is blank because it has no agentmemory variant). Bars \emph{below} the thick zero line are efficiency \emph{gains} (evolved side uses fewer turns --- \textbf{better}); bars \emph{above} zero are regressions. Note that $\Delta$Turns/Tools/Tokens/Latency are ``lower-is-better'' whereas $\Delta$Pass (not shown, saturated) is ``higher-is-better''. The three grey bars are the pure implicit-evolution effect of each base runtime and are directly comparable as an \emph{evolution-impact} ranking (Hermes $\gg$ GenericAgent $\gtrsim$ OpenClaw); see \S\ref{sec:rq-ranking}.}
\label{fig:rq1bar}
\end{figure}
...
\centering
\small
\caption{\textbf{RQ3: Per-repeat stability across the 10 repeats.} Std is over per-repeat population $\Delta$s. The three Hermes rows are the only ones whose turns-CIs sit fully below zero, giving the strongest statistical claim; every OpenClaw and GenericAgent CI crosses zero on turns, so those small $\Delta$s are noise-dominated at 10-repeat resolution.}
\label{tab:rq3}
\resizebox{\textwidth}{!}{%
...
4 & \texttt{openclaw+openspace}   & $-3.95$  & $-6.41$  & $-7.12$  & $-7.90$  & \texttt{--} \\
5 & \texttt{genericagent}         & $-1.75$  & $-1.38$  & $-0.53$  & $-2.20$  & \texttt{--} \\
6 & \texttt{openclaw+agentmemory} & $+0.75$  & $-1.19$  & $+2.28$  & $-0.18$  & \texttt{--} \\
7 & \texttt{genericagent+active}  & $+1.92$  & $+0.92$  & $+1.68$  & $+3.17$  & \texttt{--} \\
8 & \texttt{openclaw}             & $+2.74$  & $-2.09$  & $+0.44$  & $+3.34$  & \texttt{--} \\
\bottomrule
\end{tabular}%
}
\end{table}

\textbf{Reading Table~\ref{tab:rank}.} The ranking is dominated by a \emph{base-runtime} effect, not a plugin effect: the entire Hermes family (ranks 1--3) separates from everything else by a $\sim$13pp gap, and no plugin moves a runtime across that gap. Within the Hermes cluster the three configurations are statistically indistinguishable ($\Delta$Turns\% spread $<2.5$pp, all CIs overlapping), so the top of the board should be read as ``Hermes' \texttt{review} distillation'', not ``OpenSpace won''. Only ranks 1--3 carry a repeat-stable signal (RQ3); ranks 4--8 cluster within $\pm$4pp of zero and are \emph{not} statistically separable from each other or from ``no effect'' at 10 repeats --- we print their order for completeness but caution against reading ranks 4 vs.\ 5 vs.\ 6 as a real difference. \textbf{One caveat governs the whole table:} a large $\Delta$ partly reflects how much \emph{compressible slack} the Base run carried --- Hermes' verbose baseline leaves more to cut than GenericAgent's already-terse one --- so this is a ranking of \emph{realized evolution gain on EALE}, not of the intrinsic quality of each evolution mechanism. Appendix~\ref{sec:evo-mechanisms} (Table~\ref{tab:evo-mech}) spells out exactly what each runtime does to evolve --- which hook fires, when, and what it invokes --- and shows the ranking tracks a three-way split (carry-only vs.\ per-task distillation vs.\ post-hoc reflection): only the per-task-distillation class (Hermes) produces the robust signal above.
...
\begin{enumerate}
\item \textbf{Implicit evolution's efficacy depends heavily on the runtime's warmup policy.} Hermes' \texttt{review}-driven distillation delivers a strong, repeat-stable interaction-cost reduction (turns $-17.2\%$, latency $-23.0\%$, 95\% CI fully below zero), and this holds across all three Hermes configurations. OpenClaw's plain state-accumulation policy actually \emph{regresses} slightly on turns/tokens/latency, indicating that accumulated state without an explicit distillation step is not a reliable win. GenericAgent's file-I/O policy produces a tiny effect that is not statistically distinguishable from zero at 10 repeats.
...
\item \textbf{Statistical significance is unevenly distributed.} Only the three Hermes rows show 95\% CIs on $\Delta$Turns\% that lie entirely below zero (Table~\ref{tab:rq3}); every OpenClaw and GenericAgent row's turns-CI crosses zero. We report the point estimates for completeness but recommend against reading small differences ($|\Delta| \lesssim 5\%$) as effects at 10-repeat resolution. Pass rate on EALE is saturated (all eight runs 0.99--1.00 on both Base and Loaded, largest $|\Delta\text{Pass}| = 0.36$pp), so it is retained only as a guardrail against silent regressions --- none observed.
\item \textbf{We rank evolution impact, not raw capability.} The eight runtimes share a benchmark but not a raw-capability floor, a warmup policy, or an augmentation surface, so we produce no ``best agent'' by absolute Loaded score. What we \emph{do} rank (RQ5, Table~\ref{tab:rank}) is the evolution-impact $\Delta$ --- a within-runtime quantity from which each Base has already been differenced out. On that axis the Hermes family leads decisively ($\Delta$Turns\% $\approx-17$ to $-19\%$, the only repeat-stable rows), the remaining five runtimes cluster within $\pm$4pp of zero and are statistically inseparable, and the ordering is best read as an \emph{evolution-mechanism} comparison (Hermes' \texttt{review} distillation $\gg$ everything else) rather than a product leaderboard. The one confound we flag is compressible slack: a verbose Base leaves more room for evolution to cut, so large $\Delta$s are partly a property of the Base, not only of the evolution mechanism.
\end{enumerate}
...
\paragraph{Three mechanism classes, and why the ranking follows them.} The eight runtimes fall into three families of mechanism. \textbf{(C) Carry-only} (\texttt{openclaw}, \texttt{+openspace}, \texttt{+agentmemory}, and \texttt{genericagent}): there is \emph{no} distillation hook at all; the delta is simply whatever memory/skill files the agent happened to write during warmup, snapshotted by \texttt{docker commit}. \textbf{(D) Per-task distillation} (the entire Hermes family): after \emph{every} warmup task, before the next one starts, a blocking \texttt{review} compresses that task's trajectory into a compact reusable skill/memory. \textbf{(R) Explicit post-hoc reflection} (\texttt{genericagent+active}): two additional reflection chats are spawned to write memory after each task and after the batch. Mapping this onto RQ5, the only class that produces a robust, repeat-stable, double-digit efficiency gain is \textbf{(D)}: per-task distillation is precisely the step that turns a verbose warmup transcript into a short plan the model consults on Loaded runs. Class \textbf{(C)} has no compression step, so its deltas are neutral or even mildly negative (accumulated state lengthens context without shortening the outer loop); class \textbf{(R)}'s extra deliberation \emph{adds} interaction cost on a benchmark where the base already writes focused deliverables. The RQ5 ordering is therefore not an accident of tuning --- it tracks whether a runtime performs distillation at all, and when.
