...
\item \textbf{Bounded by a shared \texttt{max\_turns}.} The loop terminates when the judge marks success or the budget is exhausted; \texttt{baseline} and \texttt{evolved} see the same budget, so extra rounds cannot masquerade as artifact merit.
\end{itemize}
...
\begin{itemize}
\item \textbf{Code}: framework, twelve runtime adapters, status dashboard, post-processing --- released at \url{https://github.com/FeiZhuNiU-INFJA/LIFT}.
