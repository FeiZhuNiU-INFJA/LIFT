...
\texttt{openclaw\_with\_evolve} & with-evolve image & runs \texttt{openclaw learn review} in-container after warmup, then commit \\
\texttt{openclaw\_with\_openspace} & with-openspace image (OpenSpace MCP skill hub) & reuses base warmup / commit flow; MCP-side skill hub is exercised during warmup \\
\texttt{multi\_user\_openclaw} & base image + GroupMemoryMixin & multi-container warmup (simulated multi-user); artifacts to external group memory \\
\texttt{genericagent} / \texttt{genericagent\_active\_evolve} & \texttt{lift-genericagent:latest} & baseline file I/O; active variant does reflection chat \\
\texttt{hermes} & \texttt{lift-hermes:latest} & Hermes review flow writes \texttt{/opt/hermes-state}, then commit \\
\texttt{hermes\_with\_openspace} & \texttt{lift-hermes-with-openspace:latest} & Hermes review flow + OpenSpace MCP registered in \texttt{config.yaml}; commit \\
\texttt{openhuman} & \texttt{lift-openhuman:latest} & Rust JSON-RPC runtime; long-term memory / wiki paths enter the delta \\
\texttt{evoscientist} / \texttt{evoscientist\_active\_evolve} & \texttt{lift-evoscientist:latest} & baseline captures natural state change; active variant triggers EvoMemory AutoSkills, then commit \\
\bottomrule
\end{tabular}
\end{table}
...
Structured export (YAML/JSON artifact pack) & readable, versionable & hard to cover FS-level evolution (OpenClaw's entire \texttt{\textasciitilde/.openclaw/} tree) \\
...
Post-process & Langfuse trace backfill, comparison CSV, trajectory judging, HTML report & \texttt{*\_backfilled.json}, \texttt{*\_comparison\_metrics.csv} \\
\bottomrule
\end{tabular}
\end{table}
...
\midrule
Pipeline orchestration & \texttt{src/lift/pipeline/lift\_pipeline.py} & repeat $\times$ suite $\times$ phase multi-level concurrency \\
Adapter contract (base) & \texttt{src/lift/adapters/base.py} & abstract \texttt{AgentRuntimeAdapter}: \texttt{worker\_judger\_factory} / \texttt{evolve\_after\_warmup} hooks \\
Container orchestration & \brk{src/lift/adapters/container/adapter.py} & \brk{ContainerAgentRuntimeAdapter}: \texttt{resolve\_docker\_image} / \texttt{start\_container} + default \texttt{docker commit} \\
Per-task kernel & \texttt{src/lift/eval/run\_task.py} & work + judge review loop \\
Container policies & \texttt{src/lift/policies/container.py} & warmup / holdout / phase orchestration enums \\
Status visualization & \texttt{src/lift/status/} & TUI (rich) + HTTP dashboard \\
Trace backfill & \texttt{src/postprocess/trace\_backfill.py} & \texttt{session\_id} correlation (calls \texttt{stitch\_phase\_langfuse\_traces}) \\
