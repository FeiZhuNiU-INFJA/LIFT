...
% runs (5 runtimes × RQ1..RQ4). Numbers flagged "[Preliminary — full run pending]"
...
\emph{Preprint status.} This is a v1 preprint. §6 reports population-ratio $\Delta$ from a partial cross-runtime sweep (5 runtimes across RQ1--RQ4); remaining runtimes and full-suite repeats will be released in v2.
\end{abstract}
...
\item \textbf{Delta-of-delta within a base family (RQ2, Table~\ref{tab:rq2}).} To ask ``does the augmentation plugin add value?'', we hold the base runtime fixed and subtract two Base-vs-Loaded deltas: $\text{Gain} = \Delta_\text{augmented} - \Delta_\text{base}$. This cancels the raw-capability confound of the underlying LLM/runtime because both sides share the same Base column.
...
\end{enumerate}
We do \textbf{not} directly rank ``\texttt{openclaw+openspace} vs. \texttt{genericagent}'' in any table --- those two rows may not be subtracted, only observed side by side.
...
\textbf{Pass rate is saturated on EALE.} Across all five runs, Base pass rate is 0.99--1.00 and Loaded pass rate is 0.99--1.00 (Table~\ref{tab:rq1}). Pass rate is therefore a low-resolution signal of self-evolution on this benchmark; the primary lens is interaction efficiency, with pass rate reported for transparency and to flag any regression.
...
\textbf{Reading Table~\ref{tab:rq1}.} \texttt{hermes} shows the strongest interaction-cost reduction on every axis (turns~$-17.2\%$, latency~$-23.0\%$, tokens~$-17.5\%$); its \texttt{review}-driven implicit evolution distills verbose warmup into a compact plan that the model consults on Loaded runs. \texttt{genericagent} moves only marginally in the efficient direction ($-1$ to $-2\%$) --- its file-I/O policy captures skill files but the base runtime already writes highly focused deliverables, leaving little conversational cost to cut. \texttt{openclaw} slightly regresses on turns/tokens/latency ($+0.4\%$ to $+3.3\%$) even while trimming tool calls modestly ($-2.1\%$): natural memory accumulation without an explicit distillation hook produces state that lengthens rather than shortens the outer loop. Pass rate is essentially unchanged across all five runs ($|\Delta\text{Pass}| \le 0.36$pp), consistent with a saturated benchmark.

\subsection{RQ2 --- Do augmentation plugins add value on top of implicit evolution?}
\label{sec:rq2}

Both \texttt{openclaw} and \texttt{hermes} are compared against their \texttt{+openspace} sibling. The Gain column is a \emph{delta-of-delta}: $\text{Gain} = \Delta_\text{augmented} - \Delta_\text{base}$, in the same units as the underlying $\Delta$. A \emph{negative} Gain means the augmentation improves the metric \emph{beyond} implicit evolution alone --- i.e., the plugin adds real value on top of natural state accumulation.

\begin{table}[h]
\centering
\small
\caption{\textbf{RQ2: OpenSpace augmentation gain (delta-of-delta, same base runtime).} Negative Gain $=$ augmentation is more efficient than the implicit-only baseline.}
\label{tab:rq2}
\resizebox{\textwidth}{!}{%
\begin{tabular}{lrrrrrrrrr}
\toprule
Base family & \multicolumn{3}{c}{Turns} & \multicolumn{3}{c}{Tool calls} & \multicolumn{3}{c}{Tokens} \\
\cmidrule(lr){2-4}\cmidrule(lr){5-7}\cmidrule(lr){8-10}
             & $\Delta$Base & $\Delta$Aug & Gain    & $\Delta$Base & $\Delta$Aug & Gain    & $\Delta$Base & $\Delta$Aug & Gain    \\
\midrule
\texttt{openclaw} & $+2.74$  & $-3.95$  & $-6.69$ & $-2.09$  & $-6.41$  & $-4.32$ & $+0.44$  & $-7.12$  & $-7.55$ \\
\texttt{hermes}   & $-17.24$ & $-19.09$ & $-1.85$ & $-10.38$ & $-10.62$ & $-0.24$ & $-17.48$ & $-19.27$ & $-1.79$ \\
\bottomrule
\end{tabular}%
}
\end{table}

\textbf{Reading Table~\ref{tab:rq2}.} OpenSpace adds value on \emph{both} base families, but the mechanism differs by starting point:
\begin{itemize}
\item On \texttt{openclaw}, implicit evolution is neutral-to-negative on turns/tokens/latency (Table~\ref{tab:rq1}); OpenSpace turns this around and delivers the largest observed gain in the paper ($-6.7$pp turns, $-7.6$pp tokens). The plugin is doing real work here --- structured skill retrieval is filling in for the missing distillation step in OpenClaw's implicit-only path.
\item On \texttt{hermes}, implicit evolution is already very strong; OpenSpace's incremental gain is $-2$pp on turns/tokens and near-zero on tool calls. There is little headroom left: the base runtime has already extracted most of the compressible cost from its warmup, and adding a skill hub only marginally sharpens it.
\end{itemize}
The pattern is directionally consistent (both Gains negative on turns and tokens) but the magnitudes differ by 3--4$\times$, showing that augmentation value is inseparable from the base runtime's warmup policy --- a plugin's absolute effect cannot be quoted without naming its host.
...
\centering
\small
\caption{\textbf{RQ3: Per-repeat stability across the 10 repeats.} Std is over per-repeat population $\Delta$s. Bold-worthy: Hermes and Hermes+OpenSpace turns-CIs sit fully below zero, giving the strongest statistical claim; OpenClaw and GenericAgent CIs cross zero on turns, so those small $\Delta$s are noise-dominated at 10-repeat resolution.}
\label{tab:rq3}
\resizebox{\textwidth}{!}{%
...
Only the two Hermes rows deliver a robust turns-reduction signal at 10-repeat resolution (both CIs well below zero). \texttt{openclaw+openspace}'s CI $[-8.3,\ +0.7]$ narrowly straddles zero: the Gain in Table~\ref{tab:rq2} is real in aggregate but only marginally repeat-stable. \texttt{openclaw} and \texttt{genericagent} CIs cross zero on both turns and tools, so we treat their small point estimates as directional only, not statistically significant.
...
The Base-turns column is remarkably flat across runtimes (13.9--16.9), so the $\Delta$s translate almost directly to absolute turn-count savings: Hermes saves $\approx$3 turns per task, its OpenSpace variant $\approx$3.1, OpenClaw+OpenSpace $\approx$0.6, GenericAgent $\approx$0.2. Latency savings track turns closely on Hermes (both around $-20\%$) but decouple on OpenClaw variants where OpenSpace saves latency ($-7.9\%$) faster than it saves turns ($-4.0\%$) --- consistent with the skill hub replacing sequential tool-call chains by a single retrieved skill invocation. These Loaded columns exclude \texttt{evolve\_after\_warmup} tokens (booked as training cost, \S5), so EvolutionROI derivations do not double-count.
...
\item \textbf{OpenSpace augmentation adds value on both base runtimes, but the mechanism differs.} On OpenClaw, OpenSpace \emph{rescues} an otherwise-neutral base (Gain: turns $-6.7$pp, tokens $-7.6$pp). On Hermes, OpenSpace provides a modest incremental improvement ($-2$pp on turns/tokens) on top of an already-strong base. Both Gains are negative on turns and tokens, but their magnitudes differ by 3--4$\times$ --- augmentation value is inseparable from the host runtime.
\item \textbf{Statistical significance is unevenly distributed.} Only Hermes and Hermes+OpenSpace show 95\% CIs on $\Delta$Turns\% that lie entirely below zero (Table~\ref{tab:rq3}); the other three rows' turns-CIs cross zero. We report the point estimates for completeness but recommend against reading small differences ($|\Delta| \lesssim 5\%$) as effects at 10-repeat resolution.
\item \textbf{Pass rate on EALE is saturated.} All five runs score 0.99--1.00 on both Base and Loaded; the largest $|\Delta\text{Pass}|$ is $0.36$pp. Pass rate is retained in Tables~\ref{tab:rq1} and~\ref{tab:rq4} as a guardrail against silent regressions --- none observed --- but is unfit as a discriminator of evolution quality on this benchmark.
\item \textbf{Non-finding: no cross-runtime ranking is claimed.} The five rows of Table~\ref{tab:rq1} share a benchmark but not a raw-capability floor, a warmup policy, or an augmentation surface. We deliberately produce no ``winner'' across runtimes; each row is a self-comparison, and Table~\ref{tab:rq2} is the only cross-configuration inference we authorize (delta-of-delta, same base family).
\end{enumerate}
