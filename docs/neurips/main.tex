...
Runtimes entering the leaderboard: \emph{Implicit} --- \texttt{openclaw}, \texttt{genericagent}, \texttt{hermes}, \texttt{openhuman}, \texttt{evoscientist}; \emph{Augmented} --- \texttt{openclaw\_with\_openspace}, \texttt{openclaw\_with\_agentmemory}, \texttt{genericagent\_active\_evolve}, \texttt{hermes\_with\_openspace}, \texttt{hermes\_with\_agentmemory}, \texttt{openhuman\_with\_agentmemory}, \texttt{evoscientist\_active\_evolve}.
...
\textbf{RQ2 --- Do augmentation hooks (skill hub / external memory / active reflection) add value on top of implicit evolution?} \texttt{openclaw} vs. its augmented siblings (\texttt{openclaw\_with\_openspace}, \texttt{openclaw\_with\_agentmemory}); \texttt{hermes} vs. \texttt{hermes\_with\_openspace} / \texttt{hermes\_with\_agentmemory}; \texttt{openhuman} vs. \texttt{openhuman\_with\_agentmemory}; \texttt{genericagent} vs. \texttt{genericagent\_active\_evolve}; \texttt{evoscientist} vs. \texttt{evoscientist\_active\_evolve}. Each pair holds the base runtime fixed and toggles exactly one augmentation. \texttt{[TODO-DATA]}
