...
| `prime_agent_active_evolve` | `lift-runid-prime-full-r10` | ✅ | ⚠️ | ✅ | ✅ | 10-repeat; per-task `/refine --global` distillation. **Only the `+active` form is evaluable: prime-agent's Continual Harness is session-local by default — ordinary task execution writes a harness bound to that conversation's session id, which LIFT's fresh-container/fresh-session holdout cannot read, so a passive `docker commit` carries ≈0 usable delta. `/refine --global` promotes the increment to the session-independent layer (`~/.prime/agent/harness/`) that the commit can carry; hence a passive prime-agent variant is a near-zero negative control, not a second data row.** Not paired in RQ2 (no passive-baseline variant swept). ΔTurns% $-3.27$, 95% CI $[-6.96, +0.87]$ crosses zero (directional only). Base pass 0.9321 (below the 0.99–1.00 band) is a run-condition artifact of this sweep's timeout/hang incidents, not a capability gap |
