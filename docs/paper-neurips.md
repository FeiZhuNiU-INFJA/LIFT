...
*Preprint status.* This is a v1 preprint. §6 reports population-ratio Δ from a partial cross-runtime sweep (11 runtimes across RQ1–RQ5); remaining runtimes and full-suite repeats will be released in v2.
...
**Runs.** We report eleven full runs on the EALE benchmark, organized into base-runtime families so every cross-runtime observation stays inside a controlled comparison: **OpenClaw family** — `openclaw` (implicit), `openclaw_with_openspace` (OpenSpace skill hub), `openclaw_with_agentmemory` (agentmemory backend); **Hermes family** — `hermes` (implicit `review`), `hermes_with_openspace`, `hermes_with_agentmemory`; **GenericAgent family** — `genericagent` (implicit file-I/O), `genericagent_active_evolve` (extra active-reflection pass); **EvoScientist family** — `evoscientist` (implicit always-on internal memory loop), `evoscientist_active_evolve` (extra AutoSkills distillation pass); and **prime-agent** — `prime_agent_active_evolve` (per-task `/refine --global` distillation into a global Continual Harness). Each run covers 14 scenes × 2 holdout tasks × `--repeat 10`, yielding ≈280 (Base, Loaded) pairs per runtime. The agent model, work/judge endpoints, `max_conversation_turns=36`, and container resource passthrough are held identical across all cells; the only manipulated axis is Base vs. Loaded. This grouping lets us read three orthogonal contrasts off the tables: **(a) evolution-strategy** (implicit accumulation vs. an explicit plugin/active pass within one base), **(b) plugin** (OpenSpace vs. agentmemory on the *same* base), and **(c) base agent** (OpenClaw vs. Hermes vs. GenericAgent vs. EvoScientist vs. prime-agent, read only through delta magnitudes, never absolute scores).

**How we compare across eleven heterogeneous runtimes.** A naïve leaderboard on absolute Loaded scores conflates (a) the underlying runtime's raw capability, (b) its warmup accumulation policy, and (c) its augmentation plugin — three confounds that make head-to-head rankings scientifically empty. LIFT deliberately restricts cross-runtime inference to *delta magnitudes*, and we further partition the analysis into three tables that respect three distinct comparison boundaries:
...
**Pass rate is saturated on EALE.** Across ten of the eleven runs Base and Loaded pass are both 0.99–1.00; the exception is `prime-agent+active` (0.9321 / 0.9429), a run-condition artifact of that sweep's timeout/hang incidents discussed in §6.2 (Table 6.1). The primary lens is interaction efficiency, with pass rate reported as a regression guardrail.
...
| `evoscientist` | 280 (248) | 1.0000 | 0.9929 | −0.71 | **−14.33** | **−34.65** | **−10.23** | **−15.05** |
| `evoscientist+active` | 278 (250) | 1.0000 | 1.0000 | +0.00 | **−14.69** | **−27.57** | **−12.73** | **−16.57** |
| `prime-agent+active` | 280 (260) | 0.9321 | 0.9429 | +1.07 | −3.27 | +5.66 | +3.21 | −5.73 |

**Reading Table 6.1.** The `hermes` family shows the strongest interaction-cost reduction on every axis (turns −16.6 to −19.1%, latency ≈−22 to −23%, tokens ≈−17 to −19%); its `review`-driven implicit evolution distills verbose warmup into a compact plan the model consults on Loaded runs. The `genericagent` family moves only marginally (|Δ| ≤ 3% on all axes, either direction): its file-I/O policy captures skill files but the base already writes focused deliverables, and the extra active pass even drifts slightly inefficient (+1.9% turns). The `openclaw` family is the split case: the base and its agentmemory variant hover near zero or slightly regress (+0.75 to +3.3% on turns/tokens/latency), whereas OpenSpace is the only OpenClaw configuration that turns clearly efficient (−4.0% turns, −7.1% tokens) — accumulation without a distillation hook lengthens rather than shortens the outer loop, and an agentmemory backend does not by itself fix this. `evoscientist` is the standout non-Hermes row: it cuts turns −14.3%, tokens −10.2%, latency −15.1%, and posts the single largest tool-call reduction in the paper (−34.7%) — its always-on internal memory loop writes synthesized profiles during warmup, and its very verbose Base (186 tool-calls/task, Table 6.4) leaves the most compressible slack of any runtime; its active-AutoSkills sibling `evoscientist+active` lands almost on top of it on turns (−14.7%) but with a *smaller* tool-call cut (−27.6%), so the explicit post-hoc distillation adds no efficiency over the always-on loop it sits on. Pass rate is essentially unchanged across ten of the eleven runs (|ΔPass| ≤ 0.71pp); the one row off the saturation band is `prime-agent+active` (Base 0.9321, Loaded 0.9429, ΔPass +1.07pp), whose lower absolute pass rate is a run-condition artifact of that sweep — baseline tasks lost to per-turn timeout / session-lock hangs, not benchmark difficulty — with both phases under the same timeout regime, so its within-runtime Δs remain a valid Base-vs-Loaded contrast (it posts the run's only positive ΔPass).
...
| `evoscientist` | **−13.69** | 9.52 | **[−19.59, −7.79]** | −27.89 | 22.34 |
| `evoscientist+active` | **−14.60** | 5.77 | **[−18.18, −11.02]** | −27.11 | 39.77 |
| `prime-agent+active` | −3.05 | 6.31 | [−6.96, +0.87] | +6.21 | 10.67 |

All three Hermes rows deliver a robust turns-reduction signal at 10-repeat resolution (every CI well below zero), including `hermes+agentmemory` whose tighter std (6.31) gives the narrowest interval [−20.48, −12.66]. `evoscientist` joins them as the fourth repeat-stable row: despite the widest std of the significant group (9.52), its large mean gain keeps the whole interval [−19.59, −7.79] below zero — a robust signal from a runtime with *no* per-task distillation hook, showing distillation is sufficient but not necessary. Its active sibling `evoscientist+active` is the fifth such row and its interval is the *tightest* of the two ([−18.18, −11.02], std 5.77 vs. 9.52) at a near-identical mean: the explicit post-hoc AutoSkills pass roughly halves per-repeat variance without moving the point estimate. `openclaw+openspace`'s CI [−8.3, +0.7] narrowly straddles zero: the Gain in Table 6.2 is real in aggregate but only marginally repeat-stable. Every remaining OpenClaw and GenericAgent CI crosses zero on both turns and tools — including `openclaw+agentmemory` [−4.04, +6.47], `genericagent+active` [−2.00, +6.22], and `prime-agent+active` [−6.96, +0.87] — so we treat their small point estimates as directional only. This reinforces RQ2: the Hermes family and both EvoScientist variants carry a strong repeat-stable signal, OpenClaw+OpenSpace a marginal one, and agentmemory / active reflection / prime-agent none.
...
| `evoscientist` | 15.6 | 186.1 | 7,275,312 | −14.33 | −34.65 | −10.23 | −15.05 |
| `evoscientist+active` | 15.7 | 199.9 | 7,181,170 | −14.69 | −27.57 | −12.73 | −16.57 |
| `prime-agent+active` | 16.6 | 10.7 | 2,170,430 | −3.27 | +5.66 | +3.21 | −5.73 |
...
Given the four efficiency deltas above, we can finally order all eleven runtimes on a single axis: **how much did evolution improve this runtime?** The ranking key is ΔTurns% (most negative = most improved). This is *not* a leaderboard of raw ability — each Δ is a within-runtime Base-vs-Loaded quantity, so the ordering compares *evolution gains*, never absolute capability. The **Sig.** column marks whether the ΔTurns% 95% CI (Table 6.3) lies entirely below zero.
...
| 4 | `evoscientist+active` | **−14.69** | −27.57 | −12.73 | −16.57 | ✓ |
| 5 | `evoscientist` | **−14.33** | −34.65 | −10.23 | −15.05 | ✓ |
| 6 | `openclaw+openspace` | −3.95 | −6.41 | −7.12 | −7.90 | — |
| 7 | `prime-agent+active` | −3.27 | +5.66 | +3.21 | −5.73 | — |
| 8 | `genericagent` | −1.75 | −1.38 | −0.53 | −2.20 | — |
| 9 | `openclaw+agentmemory` | +0.75 | −1.19 | +2.28 | −0.18 | — |
| 10 | `genericagent+active` | +1.92 | +0.92 | +1.68 | +3.17 | — |
| 11 | `openclaw` | +2.74 | −2.09 | +0.44 | +3.34 | — |

**Reading Table 6.7.** The ranking is dominated by base family, not by plugin: the three Hermes configurations sweep ranks 1–3, and the two EvoScientist variants (ranks 4–5) sit only ≈2–2.5pp behind them, so the top five rows are the repeat-stable group and the remaining six are packed within ±4pp of zero. Two cautions are essential. (i) **Only ranks 1–5 carry a repeat-stable signal** — every other row's ΔTurns% CI crosses zero (Table 6.3), so ranks 6–11 are statistically inseparable and their exact order should not be over-read. (ii) **Large Δs partly reflect *compressible slack*** in the Base: a verbose base leaves more room for evolution to cut (EvoScientist's 186 tool-calls/task Base is the most verbose we measured), while `prime-agent+active` sits at rank 7 despite running the same per-task-distillation family as Hermes precisely because its Base is the *tersest* measured (10.7 tool-calls/task) with almost no slack to cut. So this is a ranking of *realized evolution gain on EALE*, not of intrinsic mechanism quality. Read that way, the table's strong claim is that a robust double-digit efficiency gain arises from more than one mechanism — Hermes' `review`-driven per-task distillation *and* EvoScientist's always-on internal synthesis — so distillation is one sufficient route rather than the only one.
...
3. **Explicit / active evolution is not automatically better than implicit.** GenericAgent's extra active-reflection pass *costs* interaction budget rather than saving it (Gain: turns +3.7pp, tokens +2.2pp): where the base already writes focused deliverables, the added deliberation is pure overhead. EvoScientist makes the point from the opposite direction: layered on a base whose always-on loop *already* evolves strongly, its active-AutoSkills pass adds *no* separable gain (turns −14.7% vs. −14.3%, statistically tied; tool calls actually *worse*, −27.6% vs. −34.7%) and its one measurable effect is to halve per-repeat variance (RQ3 std 5.77 vs. 9.52). More evolution machinery does not imply more downstream efficiency; at best it buys stability once an implicit mechanism is already saturating the available slack.
4. **Statistical significance is unevenly distributed, and not confined to one mechanism.** Five rows show 95% CIs on ΔTurns% that lie entirely below zero (Table 6.3): the three Hermes configurations *and both* EvoScientist variants; every OpenClaw, GenericAgent, and prime-agent row's turns-CI crosses zero. The passive `evoscientist` performs no per-task distillation yet is repeat-stable, so distillation is sufficient but not necessary; conversely `prime-agent+active` *does* run per-task distillation yet stays non-significant (its Base is too terse to leave compressible slack). Small differences (|Δ| ≲ 5%) should not be read as effects at 10-repeat resolution. Pass rate on EALE is saturated (0.99–1.00 on both Base and Loaded for ten of eleven runs, largest |ΔPass| = 1.07pp on the one off-band `prime-agent+active` row), retained only as a regression guardrail — none observed.
5. **We rank evolution impact, not raw capability.** The eleven runtimes share a benchmark but not a raw-capability floor, a warmup policy, or an augmentation surface, so we produce no "best agent" by absolute Loaded score. What we *do* rank (RQ5, Table 6.7) is the evolution-impact Δ — a within-runtime quantity from which each Base has already been differenced out. On that axis the Hermes family leads (ΔTurns% ≈ −17 to −19%) with both EvoScientist variants close behind (−14.3 to −14.7%); these five are the repeat-stable rows, the remaining six cluster within ±4pp of zero and are statistically inseparable. The ordering is best read as an *evolution-mechanism* comparison but not a single-mechanism story: robust gains come both from Hermes' per-task `review` distillation and from EvoScientist's always-on internal memory synthesis. The one confound we flag is compressible slack: a verbose Base leaves more room for evolution to cut.
