...
def _build_judge_prompt(user_prompt: str, agent_result: str, content_reqs: str) -> str:
    ...
    return (
        "你是用户本人，不是评测器。请站在【用户提示词】发出者的角度，看看 agent 这次"
        "做的成果跟你心里要的对不对得上，给一句你会真说出来的反馈。\n"
        ...
        "- 只挑当下最该改的一个点说，不要罗列两条以上未满足项；\n"
        "- 不要把材料目录、文件名、路径整段搬出来——agent 自己看得到，写出来反而像 checklist；\n"
        "- 已经做对的不用刻意夸，除非 agent 明显走偏需要肯定他这一步是对的；\n"
        "- 用平实的话告诉他「这次哪里不对」「下次试试怎么做」就行，像同事顺口提醒，不像审计员复盘。\n"
        "\n"
        "其它硬约束：\n"
        "- 反馈要让 agent 看完知道下一步具体怎么改（agent 看不到任务期望和要求的全文）；\n"
        ...
        '你需要先读取……再按照……生成……保存到……"\n'
        '- 拟人版（正确示范）："你这把好像啥都没碰，材料也没读、结果文件也没出。先把那几个 xlsx 打开看下吧。"\n'
        "\n"
        f"【用户提示词】\n{user_prompt}\n\n"
